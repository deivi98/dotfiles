...
group_names = [
    ("1", {'layout': 'columns', 'matches': [    # Browser
        Match(wm_class='chromium'),
        Match(wm_class='firefox'),
        Match(wm_class='qutebrowser')
    ]}),
    ("2", {'layout': 'columns', 'matches': [    # Files
    ...
    ("3", {'layout': 'columns', 'matches': [    # Editor
        Match(wm_class='code'),
        Match(title='LunarVim'),
        Match(title='Qtile config')
    ]}),
    ("4", {'layout': 'stack', 'matches': [       # Discord
